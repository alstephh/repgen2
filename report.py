...
def parse_severity_SSVC(content):
  
  regex = r"(?P<category>\w+)\s+(?P<ssvc_string>SSVCv2\/(?P<ssvc>[^\n]*))"
  match = re.search(regex, content, flags=re.MULTILINE)
  ...
  if match:
    return {
      'value' : cat_to_values[match.group('category')],
      'category': match.group('category'),
      'ssvc': parse_severity_vector(match.group('ssvc'))
    }
      ...
      if section_level == 1:
        ...
        if section_name == 'severity':
          if SCORE_TYPE == 'CVSS':
            ...
            sections[section_name] = parse_severity_SSVC(section_content)  
...
### WE CAN USE THIS SPACE FOR MISSING SEVERITIES ERROR
...
def main():
  ...
  parser.add_argument(
    '-s', '--scoretype',
    help='Choose vulnerability score : [CVSS, SSVC]',
    choices=['CVSS', 'SSVC'],
    required=True
  )
